...
const NavigationBtn: React.FC<Props> = (props) => {
    ...
    return (
        <button onClick={navigateHandler} className='md:flex md:gap-4 md:items-center md:hover:bg-slate-100/5 md:w-full md:p-2 md:rounded-lg md:ease-in-out duration-200' disabled={steps.currentStep === 5}>
            <span className={`rounded-full h-10 w-10 border border-teal-100 font-medium text-white ease-in-out duration-300 ${(steps.currentStep === props.currentStep || (props.currentStep === 4 && steps.currentStep === 5)) && 'bg-teal-100 text-slate-900'} flex justify-center items-center`}>
                {props.currentStep}
            </span>
